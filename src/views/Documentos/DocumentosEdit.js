...
export default function DocumentosEdit(props) {
  const [activeIndex, setActiveIndex] = useState(0);
  const [animating, setAnimating] = useState(false);
  const [documentoImages, setDocumentoImages] = useState([]);
  const [documento, setDocumento] = useState(documentoInitialState);
  const [formValidate, setFormValidate] = useState(formValidateInitialState);
  const [uploadedFiles, setUploadedFiles] = useState([]);
  const [optionStatusdocumento, setStatusdocumento] = useState({});
  const id =
  ...
  async function getDocumento() {
    try {
      if (id) {
        ...
        setStatusdocumento(
          optionsStatusdocumento.find((element, index, array) => {
  ...
  async function createdocumento() {
    try {
      ...
      setDocumento(documentoInitialState);
      props.history.push({ pathname: "/console/documento" });
    } catch (e) {}
  }
  ...
  async function updatedocumento() {
    try {
      const data = {
        id: id,
        id_status: optionStatusdocumento.value,
        assunto: documento.assunto,
        descricao: documento.descricao,
        id_album: urlImage,
      };
                ...
                <Row>
                  <Col xs="12" sm="6" md="4">
                    <FormGroup>
                      ...
                      <Select
                        options={optionsStatusdocumento}
                        placeholder="Selecione..."
                        onChange={(selectedOption) => {
                          setStatusdocumento(selectedOption);
                        }}
                        value={optionStatusdocumento}
                        theme={(theme) => ({
                          ...theme,
                          colors: {
                            ...theme.colors,
