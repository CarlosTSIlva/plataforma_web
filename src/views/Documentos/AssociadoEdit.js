...
export default function AssociadoEdit(props) {
  ...
  useEffect(() => {
    setTipoAssociado({})
    if (optionUnidadeHabitacional && optionUnidadeHabitacional.type === 'Administracao') {
      setOptionsTipoAssociado(optionsTipoAssociadoAdministracao)
    } else if (optionUnidadeHabitacional && optionUnidadeHabitacional.type === 'Residencial') {
  ...
  async function getAssociado() {
    try {
      if (id) {
        ...
        setUnidadeHabitacional(_optionsUnidadeHabitacional.find((element, index, array) => {
          if (element.value === response.data.data.unidade.id)
            return element
          return false
        }))
        setStatusAssociado(optionsStatusAssociado.find((element, index, array) => {
          if (element.value === response.data.data.status.id)
            return element
          return false
        }))
        setTipoAssociado(_optionsTipoAssociado.find((element, index, array) => {
          if (element.value === response.data.data.tipo.id)
            return element
          return false
        }))
        setSexo(optionsSexo.find((element, index, array) => {
  ...
  async function createAssociado() {
    try {
      const data = {
        ...
        id_conta_pai: optionAssociadoTitular ? optionAssociadoTitular.value : null,
        username: associado.usuario.username,
        password: associado.usuario.password,
        id_genero: optionSexo ? optionSexo.value : null,
        data_nascimento: associado.usuario.data_nascimento ? moment(associado.usuario.data_nascimento).format('YYYY-MM-DD') : null
      }
        ...
        try {
          ...
          if (response && response.data) {
            if (response.data.exists) {
              setFormValidate({ ...formValidate, username: "Este Usuário já esta sendo utilizado, por gentileza tente outro.", username_invalid: true })
            } else {
              setFormValidate({ ...formValidate, username: usernameValidate, username_invalid: false })
            }
          }
        } catch (e) {
          setFormValidate({ ...formValidate, username: "Não foi possível verificar este usuário.", username_invalid: true })
        }
      }
    }
  }
    ...
    if (mode !== 'update') {
      ...
      if (usernameValidate) {
        ...
        try {
          ...
          if (response && response.data) {
            if (response.data.exists) {
              setFormValidate({ ...formValidate, email: "Este E-mail já esta sendo utilizado, por gentileza tente outro.", email_invalid: true })
            } else {
              setFormValidate({ ...formValidate, email: usernameValidate, email_invalid: false })
            }
          }
        } catch (e) {
          setFormValidate({ ...formValidate, email: "Não foi possível verificar este email.", email_invalid: true })
        }
      }
    }
  }
    ...
    <div className="animated fadeIn">
      <Row>
        <Col xs="12" sm="12">
          <Card>
            <CardHeader>
              <i className="fa fa-edit"></i><strong>Conta</strong>
            </CardHeader>
            <CardBody>
              <Row>
                <Col xs="12" sm="6" md="4">
                  <FormGroup>
                    ...
                    <Select
                      ...
                      onChange={selectedOption => { setUnidadeHabitacional(selectedOption) }}
                      theme={(theme) => ({
                        ...theme,
                        colors: {
                          ...theme.colors,
                ...
                <Col xs="12" sm="6" md="4">
                  <FormGroup>
                    ...
                    <Select
                      ...
                      onChange={selectedOption => { setTipoAssociado(selectedOption) }}
                      theme={(theme) => ({
                        ...theme,
                        colors: {
                          ...theme.colors,
              ...
              <Row>
                <Col xs="12" md="8">
                  <FormGroup>
                    ...
                    <Input type="text" placeholder="Preencha o nome."
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, nome: e.target.value } }) }}
                      value={associado?.usuario?.nome ? associado.usuario.nome : ""}
                      onBlur={() => { setFormValidate({ ...formValidate, nome: validatefield('nome', associado?.usuario?.nome?.trim()) }) }}
                ...
                <Col xs="12" md="4">
                  <FormGroup>
                    ...
                    <Input type="date"
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, data_nascimento: e.target.value } }) }}
                      value={associado?.usuario?.data_nascimento ? moment(associado.usuario.data_nascimento).utc().format('YYYY-MM-DD') : "1900-01-01"}
                      onBlur={() => { setFormValidate({ ...formValidate, data_nascimento: validatefield('data', associado?.usuario?.data_nascimento) }) }}
              ...
              <Row>
                <Col xs="12" md="4">
                  <FormGroup>
                    ...
                    <Input type="text" id="descricao" placeholder="Preencha o documento..."
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, documento_identificacao: e.target.value } }) }}
                      value={associado?.usuario?.documento_identificacao ? associado.usuario.documento_identificacao : ""}
                      required />
                  </FormGroup>
                </Col>
                <Col xs="12" md="4">
                  <FormGroup>
                    <Label>E-mail</Label>
                    <Input type="email" placeholder="Preencha o e-mail..."
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, email: e.target.value } }) }}
                      value={associado?.usuario?.email ? associado.usuario.email : ""}
                      onBlur={() => { handleEmail(associado?.usuario?.email) }}
                      invalid={formValidate.email ? true : false}
                      valid={mode === 'update' ? false : formValidate.email_invalid ? false : true}
                    />
                    <FormFeedback valid>Legal! este E-mail está disponível.</FormFeedback>
                ...
                <Col xs="12" md="4">
                  <FormGroup>
                    ...
                    <Input type="text" placeholder="Preencha o telefone celular..."
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, telefone: e.target.value } }) }}
                      value={associado?.usuario?.telefone ? associado.usuario.telefone : ""}
                      required />
                  </FormGroup>
                </Col>
              </Row>
                ...
                <Col xs="12" sm="6" md="4">
                  <FormGroup>
                    ...
                    <Select
                      ...
                      onChange={selectedOption => { setStatusAssociado(selectedOption) }}
                      theme={(theme) => ({
                        ...theme,
                        colors: {
                          ...theme.colors,
                    ...
                    />
                    <FormText style={{ color: "#ff0000" }}>(*) Campo obrigatório.</FormText>
                  </FormGroup>
                </Col>
                <Col xs="12" sm="6" md="4">
                  <FormGroup>
              ...
              <Row>
                <Col xs="12" sm="6" md="4">
                  <FormGroup>
                    <Label>Usuário</Label>
                    <Input type="text" placeholder="Preencha o usuário..."
                      onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, username: e.target.value } }) }}
                      value={associado?.usuario?.username ? associado.usuario.username : ""}
                      disabled={mode === "insert" ? false : true}
                      onBlur={() => { handleUsername(associado?.usuario?.username) }}
                      invalid={formValidate.username ? true : false}
                      valid={mode === 'update' ? false : formValidate.username_invalid ? false : true}
                    />
                    <FormFeedback valid>Legal! este Usuário está disponível.</FormFeedback>
                    ...
                    (
                      <FormGroup>
                        ...
                        <Input type="password" placeholder="Preencha a senha..."
                          onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, password: e.target.value } }) }}
                          value={associado?.usuario?.password ? associado?.usuario?.password : ""}
                          onBlur={() => { setFormValidate({ ...formValidate, password: validatefield('password', associado?.usuario?.password) }) }}
                    ...
                    (
                      <FormGroup>
                        ...
                        <Input type="password" placeholder="Confirme a senha..."
                          onChange={(e) => { setAssociado({ ...associado, usuario: { ...associado.usuario, password_valid: e.target.value } }) }}
                          value={associado?.usuario?.password_valid ? associado.usuario.password_valid : ""}
                          onBlur={() => {
                            if (associado?.usuario?.password_valid !== associado?.usuario?.password) {
                              setFormValidate({ ...formValidate, confirm_password: "A confirmação de senha não conferere com a senha digitada." })
                            } else {
                              setFormValidate({ ...formValidate, confirm_password: null })
                            }
                          }}
                          invalid={formValidate.confirm_password ? true : false} />
                        <FormFeedback>{formValidate.confirm_password}</FormFeedback>
                      </FormGroup>
                    ) : null
                  }
                </Col>
            ...
            <CardFooter>
              <div className="card-header-actions">
                <Button block color="primary" onClick={() => { salvarAssociado() }}>Salvar</Button>
              </div>
            </CardFooter>
          </Card>
        </Col>
